...
### 3. Поднять внешние зависимости

Нужно отдельно поднять PostgreSQL и RabbitMQ.
...
### 6. Запустить consumer во втором терминале

Если установлен только базовый пакет FastStream без CLI extras, используйте:

```bash
python -c "import asyncio; from consumer.main import app; asyncio.run(app.run())"
```

Если установлен CLI:

```bash
faststream run consumer.main:app
```
...
Проектовые тесты используют текущую PostgreSQL БД `payments`, поднятую в Docker.
...
Установите standalone Compose или плагин:
...
Также проверьте RabbitMQ UI и наличие очередей:
- `payments.new`
- `payments.new.dlq`
